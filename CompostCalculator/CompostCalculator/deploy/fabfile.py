...
def _install_supervisor():
    ...
    # )
    sudo('mv /home/ubuntu/CompostCalculator/CompostCalculator/deploy/supervisord.conf /etc/supervisor/conf.d/CompostCalculator.conf')
...
def _move_nginx_files():
    ...
    sudo('mv /home/ubuntu/CompostCalculator/CompostCalculator/deploy/simple_nginx_config /etc/nginx/sites-available/default')
...
def _mass_install():
    ...
    with settings(warn_only=True):
        sudo('pip install -r CompostCalculator/requirements.txt')
...
def _install_app():
    # sudo('mv CompostCalculator/setup.py setup.py')
    sudo('python /home/ubuntu/CompostCalculator/manage.py runserver')
...
def deploy():
    ...
    run_command_on_selected_server(
        rsync_project,
        local_dir='../../../CompostCalculator',
        remote_dir="~/",
        exclude=[
            ".git",
            "CompostCalculator/deploy/env_vars.txt",
            "CompostCalculator/tests",
        ]
    )
    mass_install()
    install_supervisor()
